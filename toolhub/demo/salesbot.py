...
def _run(query: str):
  openapi = auth.OpenApiAuthContext(
    api_to_headers={
      "crunchbase": {"X-cb-user-key": _CRUNCHBASE_KEY}
  }
  )
  auth_ctx = auth.StandardAuthContext(rapidapi=auth.RapidApiAuthContext(rapidapi_key=_RAPIDAPI_KEY), openapi=openapi)
